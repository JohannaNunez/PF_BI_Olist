...
def transfer_stg_to_prod(job_key):
    if job_key == DwTables.CUSTOMERS:
        return transfer_stg_to_prod_customers()
        
    elif job_key == DwTables.ORDER_ITEMS:
        return transfer_stg_to_prod_order_items()
        
    elif job_key == DwTables.ORDER_PAYMENTS:
        return transfer_stg_to_prod_order_payments()
        
    elif job_key == DwTables.ORDER_REVIEWS:
        return transfer_stg_to_prod_order_reviews()
        
    elif job_key == DwTables.ORDERS:
        return transfer_stg_to_prod_orders()
        
    elif job_key == DwTables.PRODUCT_CATEGORY_NAME_TRANSLATION:
        return transfer_stg_to_prod_product_category_name_translation()
        
    elif job_key == DwTables.PRODUCTS:
        return transfer_stg_to_prod_products()
